...
export const AppsProView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApps = async () => {
      ...
      try {
        ...
        console.error('Error fetching pro apps:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      <h2 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        Pro Apps <BadgeCheck size={20} color="var(--primary)" />
      </h2>
      ...
      {isLoading ? (
        ...
        <div style={{ marginTop: '40px', textAlign: 'center', opacity: 0.7, background: 'rgba(255,255,255,0.05)', padding: '40px', borderRadius: '16px' }}>
          <p>No Pro apps published yet.</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px', marginTop: '24px' }}>
          {apps.map(app => (
            <div key={app.id} className="glass" style={{ padding: '20px', borderRadius: '16px', border: '1px solid var(--primary)', display: 'flex', flexDirection: 'column', background: 'rgba(139, 92, 246, 0.05)' }}>
