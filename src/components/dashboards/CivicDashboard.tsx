...
import { SosAlertsManager } from './SosAlertsManager';
...
export const CivicDashboard: React.FC<CivicDashboardProps> = ({ user, activeTab: propTab }) => {
  ...
  const fetchIncidents = async () => {
    ...
    if (!error && data) {
      ...
      // Fallback/Mock
      setIncidents([
        { id: '1', type: 'pothole', description: 'Deep pothole on Main St', location: '123 Main St', status: 'open', priority: 'medium', created_at: new Date().toISOString(), upvote_count: 12, reporter: { name: 'Resident Jane' } },
  ...
  const navItems = [
    ...
    ...(isManager ? [{ id: 'directory', label: 'Directory', icon: <Users size={20} /> }] : []),
    { id: 'tickets', label: isWorker ? 'My Tasks' : 'Issue Tracker', icon: <History size={20} /> },
    ...(isManager ? [{ id: 'utilities', label: 'Utilities', icon: <Wallet size={20} /> }] : []),
    { id: 'alerts', label: 'Alerts', icon: <AlertTriangle size={20} /> },
    ...(isManager ? [
      { id: 'ads', label: 'Sponsorships', icon: <MessageSquare size={20} /> },
  ...
  return (
    <PremiumDashboardLayout
      title={`${user?.community || 'Civic'} ${isManager ? 'Command' : 'Staff'}`}
      icon={isManager ? <Crown size={24} /> : <ShieldAlert size={24} />}
      navItems={navItems}
      activeTab={activeTab}
    ...
    >
        ...
        {activeTab === 'alerts' && (
          <div className="fade-in">
            <SosAlertsManager user={user as User} />
          </div>
        )}
