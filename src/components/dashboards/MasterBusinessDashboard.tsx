...
export const MasterBusinessDashboard: React.FC<MasterBusinessDashboardProps> = ({ user }) => {
  ...
  useEffect(() => {
    if (stores.length > 0) {
      ...
      if (activeTab === 'team' && !selectedTeamStoreId) {
  ...
  if (isLoading) return <div className="vendor-dashboard-loading"><Loader2 className="animate-spin" size={48} /><p>Loading Portfolio Data...</p></div>;

  const navItems = [
    { id: 'overview', label: 'Portfolio Overview', icon: <BarChart3 size={20} /> },
    { id: 'talent', label: 'Talent & Jobs', icon: <Briefcase size={20} /> },
    { id: 'team', label: 'Portfolio Team', icon: <Users size={20} /> },
    { id: 'ads', label: 'Global Ads', icon: <Megaphone size={20} /> },
          ...
          <div className="master-overview">
            ...
            <div className="store-management-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20, marginTop: 16 }}>
              {storeStats.map(store => (
                <div key={store.id} className="store-stat-card premium-card glass-hover" onClick={() => window.open(`/?env=shop&store=${store.id}`)} style={{ cursor: 'pointer' }}>
                  ...
                  <div className="store-card-actions" style={{ marginBottom: 16, display: 'flex', gap: 8 }}>
                    <button 
                      ...
                      onClick={(e) => { e.stopPropagation(); window.location.href = `/?env=dashboard`; }}
